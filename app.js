...
function renderSetup() {
  ...
  document.getElementById('setup').innerHTML = `
    ...
    <div class="body">
      <div class="input-group" id="setup-inputs">
        <div>
      ...
      <div class="add-habit-row" style="margin-top:12px;">
        <button class="nav-btn" onclick="addSetupHabit()">+ Add another habit</button>
      </div>
    </div>
    <div class="nav-strip">
...
function addSetupHabit() {
  setupCount++;
  const group = document.getElementById('setup-inputs');
  const div = document.createElement('div');
  div.innerHTML = `
    <div class="input-label">Habit ${setupCount}</div>
    <input class="habit-input" id="h${setupCount}" type="text" placeholder="e.g. Read" maxlength="32">
  `;
  group.appendChild(div);
  document.getElementById(`h${setupCount}`).focus();
}
...
function saveSetup() {
  ...
  if (!vals.length) { showToast('Please name at least one habit'); return; }
  habits = vals;
  saveHabits(habits);
  viewOffset = 0;
  renderHome();
...
function renderHome() {
  ...
  document.getElementById('home').innerHTML = `
    ...
    <div class="body">${cards}<p style="color:var(--text-muted);font-size:0.85rem;margin-top:12px;text-align:center;">Tap settings to add another habit.</p></div>
    <div class="nav-strip">
